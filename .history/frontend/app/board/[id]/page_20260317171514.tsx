...
function StickyNote({
  el, zoom, pan, activeTool,
  onMove, onContentChange, onSelect, isSelected,
}: {
  el: Element; zoom: number; pan: { x: number; y: number };
  activeTool: Tool; onMove: (id: string, x: number, y: number) => void;
  onContentChange: (id: string, v: string) => void;
  onSelect: (id: string | null) => void; isSelected: boolean;
}) {
  ...
  const onMouseDown = (e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      onMove(el.id, dragStart.current.ex + dx, dragStart.current.ey + dy);
    };
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 8, cursor: activeTool === "hand" ? "grab" : "move",
        userSelect: "none", overflow: "hidden", transition: "box-shadow .15s",
      }}
    >
      {/* Top color strip */}
      ...
      <textarea
        ...
        style={{
          ...
          padding: `${8 * zoom}px`, fontSize: `${13 * zoom}px`,
          fontFamily: "inherit", resize: "none", cursor: "text",
...
export default function BoardPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const [listsData, elements] = await Promise.all([
          api.get(`/api/boards/${params.id}/lists`),
          api.get(`/api/boards/${params.id}/elements`),
        ]);
        setElements(elements);
      } catch {
        // Board may have no elements yet
      }
    })();
  }, [params.id]);
  ...
  useEffect(() => {
    ...
    socket.on("element:added", (el: Element) => setElements((p) => [...p, el]));
    socket.on("element:moved", ({ id, x, y }: any) => setElements((p) => p.map((e) => e.id === id ? { ...e, x, y } : e)));
    socket.on("element:updated", ({ id, content }: any) => setElements((p) => p.map((e) => e.id === id ? { ...e, content } : e)));
    socket.on("cursor:move", ({ email, x, y, color }: any) => {
    ...
    return () => {
      ...
      ["presence:update","element:added","element:moved","element:updated","cursor:move"].forEach((e) => socket.off(e));
    };
  ...
  const onCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    socketService.getSocket().emit("cursor:move", { boardId: params.id, email: myEmail, x: cx, y: cy, color: myColor });
  }, [params.id, myEmail, myColor]);

  // ── Pan (hand tool or middle-click) ──
  ...
  const onCanvasMouseDown = (e: React.MouseEvent) => {
    if (activeTool === "hand" || e.button === 1) {
      ...
      const onUp = () => { panDrag.current = null; window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
      window.addEventListener("mousemove", onMove);
      window.addEventListener("mouseup", onUp);
      return;
    }
  ...
  // ── Add element ──
  const addElement = async (opts: Partial<Element>) => {
    const el: Element = {
    ...
    try {
      await api.post(`/api/boards/${params.id}/elements`, el);
    } catch {}
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-white select-none overflow-hidden"
      ...
      <div className="h-11 flex items-center border-b border-gray-200 bg-white px-3 gap-2 shrink-0 z-30">
        ...
        <button className="ml-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors">
          Share
        </button>
      </div>
      ...
      <div className="h-11 flex items-center border-b border-gray-200 bg-white px-3 gap-1 shrink-0 z-30">
        ...
        <div className="flex gap-0.5 mr-2">
          <button className="p-1.5 text-gray-500 hover:bg-gray-100 rounded transition-colors">{icons.undo}</button>
          <button className="p-1.5 text-gray-500 hover:bg-gray-100 rounded transition-colors">{icons.redo}</button>
        </div>
        <div className="h-5 w-px bg-gray-200 mx-1" />
        ...
        {([
          ...
          { key: "sticky", label: "Sticky note (S)" },
          { key: "text", label: "Text (T)" },
          { key: "comment", label: "Comment (C)" },
        ...
        >
          ...
          {elements.map((el) => (
            <StickyNote
              ...
              onContentChange={(id, v) => updateContent(id, v)}
          ...
          {Array.from(cursors.values()).map(({ email, x, y, color }) => {
            ...
            return (
              <div key={email} style={{ position: "absolute", left: sx, top: sy, pointerEvents: "none", zIndex: 100, transform: "translate(-2px, -2px)" }}>
                <svg width="20" height="20" viewBox="0 0 20 20">
                  <path d="M5 3l12 7.5-7 1-3.5 6z" fill={color} stroke="white" strokeWidth="1.5"/>
                </svg>
          ...
          {elements.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm font-medium">Select the sticky note tool and click to add notes</p>
                <p className="text-gray-300 text-xs mt-1">Scroll to zoom · Hold space to pan</p>
              </div>
            </div>
          )}
        </div>
        ...
        {showPresence && onlineUsers.length > 0 && (
          <div className="absolute bottom-14 right-4 bg-white border border-gray-200 rounded-xl shadow-lg w-56 z-20 overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2.5 border-b border-gray-100">
              <span className="text-xs font-semibold text-gray-700">Live participants</span>
              <button onClick={() => setShowPresence(false)} className="text-gray-400 hover:text-gray-600">
      ...
      <div className="absolute bottom-4 left-14 flex items-center gap-1 bg-white border border-gray-200 rounded-lg shadow-sm px-1 py-0.5 z-20">
        <button onClick={() => setZoom((z) => Math.max(0.15, z - 0.1))}
          className="p-1.5 text-gray-500 hover:bg-gray-100 rounded transition-colors">{icons.minus}</button>
        ...
        <button onClick={() => setZoom((z) => Math.min(4, z + 0.1))}
          className="p-1.5 text-gray-500 hover:bg-gray-100 rounded transition-colors">{icons.plus}</button>
